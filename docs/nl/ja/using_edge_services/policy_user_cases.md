...
MMS オブジェクト内で、モデル・ポリシーは、オブジェクトにアクセスできるサービスまたはサービス・リスト (このケースでは atm1) を宣言し、さらにプロパティーおよび制約を宣言することで、{{site.data.keyword.ieam}} が、地方の場所にある ATM へのオブジェクトの適切な配置をさらに絞り込めるようにします。 ATM で実行されているその他のサービスはオブジェクトにアクセスできません。
